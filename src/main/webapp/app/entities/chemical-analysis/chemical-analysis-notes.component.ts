...
@Component({
    selector: 'jhi-chemical-analysis-notes',
    templateUrl: './chemical-analysis-notes.component.html'
})
export class ChemicalAnalysisNotesComponent implements OnInit, OnDestroy {

currentAccount: any;
    chemicalAnalyses: ChemicalAnalysisNotes[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/chemical-analysis-notes'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/chemical-analysis-notes', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId (index: number, item: ChemicalAnalysisNotes) {
        return item.id;
    }
