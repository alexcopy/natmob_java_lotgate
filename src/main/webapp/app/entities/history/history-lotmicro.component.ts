...
@Component({
    selector: 'jhi-history-lotmicro',
    templateUrl: './history-lotmicro.component.html'
})
export class HistoryLotmicroComponent implements OnInit, OnDestroy {

currentAccount: any;
    histories: HistoryLotmicro[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/history-lotmicro'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/history-lotmicro', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId (index: number, item: HistoryLotmicro) {
        return item.id;
    }
